...
select concat(first_name, ' ', last_name) as '전체 이름', gender as 성별, hire_Date as '입사 날짜' from employees;
...
-- order by
-- ex1) employees 테이블에서 직원의 전체이름,  성별, 입사일을 입사일 순(신입 먼저)으로 출력
select concat(first_name, ' ', last_name) as 이름, gender as 성별, hire_Date as '입사 날짜' from employees order by hire_date desc;
...
-- where (조건절)
-- ex1) employees 테이블에서 1991년 이전에 입사한 직원의 이름, 성별, 입사일을 출력
